...
2. Abrir o app (`iniciar.bat`) e usar o **modo demo** — visual limpo, sem sidebar de configuração:
   ...
   (pra gravar pra um corretor específico, troca o nome na URL — não precisa mexer em arquivo nenhum)
...
4. Gravação: **Win + G** (Xbox Game Bar, já vem no Windows) → gravar a janela do navegador. Dar zoom (Ctrl +) pra ficar legível no celular — corretor vai assistir no WhatsApp.

## Roteiro da conversa (você no papel do lead)

Digitar como lead real digitaria — com pressa, informal. Sequência testada (baseada nos casos do `teste_agente.py`):

1. `Oi, vi um anúncio de vocês`
2. `Meu nome é Marcos, tô procurando apartamento pra alugar`
3. `Bairro Centro ou perto, até uns 1500 por mês`
4. **A curveball (momento-chave do vídeo):** `Vocês tem apto disponível no Centro agora? Quanto tá?` → o agente deve responder que o corretor apresenta as opções da carteira, SEM inventar imóvel nem preço. **Isso é o que mata a objeção "IA vai falar besteira".**
5. `Preciso mudar até o fim do mês que vem. Pode me ligar depois das 18h`
6. Deixar o agente fechar com o **📋 RESUMO PARA O CORRETOR** — ele aparece num card verde destacado, esse frame final é o clímax do vídeo.
...
- Não inventou imóvel/preço/condição em nenhum momento?
